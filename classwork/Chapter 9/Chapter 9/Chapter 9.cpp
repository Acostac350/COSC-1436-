...
void CharFunctionDemo()
{
    ...
    for (int index = 0; index < input.length(); ++index)
    {
        ...
        // All char functions are from C and therefor accept and return an int, not a char.
...
{
    // Always initialize C strings to null
    ...
    // std::string str;
    // str = "Hello:";
    strcpy(output, input); // strcpy (target, source) :: copies source to targets // strcpy (char*, const char*)
    ...
    strncpy(output, input, MaxInputSize); // strncpy(target, source, maxsize) :: copies up to maxsize characters.
    ...
    char* pos = strstr(input, "a"); // Find substring in string, returns pointers to start of the substring or nullptr

    //Conversion
    int value = atoi("123"); // Parses string to int
    value = atoi("123abc"); // returns 123
    value = atoi("abc"); // returns 0
...
void CPlusPlusStringFunctions()
{
    ...
    std::string sub = input.substr(10); // Gets the character starting at index, up to the given length
